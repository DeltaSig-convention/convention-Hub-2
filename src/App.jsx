...
function StaffPlan({people,blocks,setBlocks,schedule,admin}){
  ...
  return <div className="space-y-5">
    ...
    {Object.keys(grouped).length===0 && <Card><p className="text-sm" style={{color:C.muted}}>No plan built for {person} yet.{admin? " Click \"Add block\" to start building their day.":""}</p></Card>}
...
function BlockRow({b,admin,onEdit,onDelete}){
  ...
  return <Card style={{borderLeft:`4px solid ${BLOCK_KIND_COLOR[b.kind]||C.muted}`}}>
    <div className="flex items-start justify-between gap-3 flex-wrap">
      ...
      {admin && <div className="flex items-center gap-2"><button onClick={onEdit} className="text-xs font-semibold flex items-center gap-1" style={{color:C.nile}}><Pencil size={12}/>Edit</button><button onClick={onDelete} style={{color:C.muted}}><Trash2 size={14}/></button></div>}
    </div>
  </Card>;
}
...
function BudgetSection({expenses,setExpenses,income,setIncome,settings,setSettings,sponsors,setSponsors,audit,tracker,setTracker,editor,headcount}){
  ...
  const SUBS=[{id:"budget",label:"Budget"},{id:"tracker",label:"Expense Tracker"},{id:"income",label:"Income"},{id:"foundation",label:"Foundation"},{id:"activity",label:"Activity"}];
  return <div className="space-y-5">
    ...
    {sub==="budget" && <Budget expenses={expenses} setExpenses={setExpenses} settings={settings} setSettings={setSettings} editor={editor}/>}
    {sub==="tracker" && <Tracker tracker={tracker} setTracker={setTracker}/>}
    {sub==="income" && <Income income={income} setIncome={setIncome} sponsors={sponsors} setSponsors={setSponsors} headcount={headcount}/>}
    {sub==="foundation" && <Foundation expenses={expenses} income={income} settings={settings}/>}
    {sub==="activity" && <Activity audit={audit} expenses={expenses} setExpenses={setExpenses} editor={editor}/>}
  </div>;
}

function Budget({expenses,setExpenses,settings,setSettings,editor}){
  ...
  return <div className="space-y-5">
    <SectionTitle sub="Edit any cell. Lines flagged “scales” track the headcount from Income automatically (quantity follows attendees). Tag grantability and submit lines for finance-committee approval.">Budget — Master Expenses</SectionTitle>
    <Card>
    ...
    {Object.entries(groups).map(([cat,rows])=>{
      const sub=rows.reduce((a,r)=>a+effEstimated(r,settings),0); const act=rows.reduce((a,r)=>a+(r.actual||0),0); const used=sub>0?act/sub:0; const over=act>sub;
      return <Card key={cat} pad={false}>
        ...
        <div className="overflow-x-auto"><table className="w-full text-sm" style={{borderCollapse:"collapse"}}>
          <thead><tr style={{color:C.muted}} className="text-[11px] uppercase tracking-wide">{["Line item","Serves","Pays","Qty","Unit","Estimate","Actual","Remaining","Grant","Approval","To students",""].map((h,i)=><th key={i} className="text-left font-semibold px-3 py-2" style={{borderBottom:`1px solid ${C.line}`}}>{h}</th>)}</tr></thead>
          <tbody>{rows.map(r=>{ const eff=effEstimated(r,settings); const rem=eff-(r.actual||0); const ss=studentShare(r,settings); const lineOver=(r.actual||0)>eff; const scaledQty=effQty(r,settings);
            return <tr key={r.id} style={{borderBottom:`1px solid ${C.line}`,background:lineOver?"#fdf3ef":"transparent"}}>
              ...
              <td className="px-3 py-2"><NumCell value={r.actual} onChange={v=>patch(r.id,"actual",v)} prefix="$" w={88}/></td>
...
function Tracker({tracker,setTracker}){
  ...
  const blank=()=>({id:uid("t"),date:"",vendor:"",cost:0,desc:"",category:"",subcategory:"",dept:"DSP",project:"Miami 2027 Convention",account:"",method:"—",pstatus:"Not started",requestedBy:"",approvedBy:"",approvalDate:"",contractReq:false,contractSigned:false,grantable:false,grantPct:0});
  ...
  const filtered=tracker.filter(r=>(statusF==="All"||r.pstatus===statusF)&&(q===""||(r.vendor+r.desc+r.category+r.account).toLowerCase().includes(q.toLowerCase())));
  ...
  const exportCSV=()=>{ const head=["Date","Vendor","Total Cost","Description","Category","Subcategory","Dept","Project","QB Account Line","Payment Method","Payment Status","Requested By","Approved By","Approval Date","Contract Req","Contract Signed","Grantable","Grant %","Grant Amount"];
    const lines=[head.join(",")].concat(tracker.map(r=>[r.date,r.vendor,r.cost,r.desc,r.category,r.subcategory,r.dept,r.project,r.account,r.method,r.pstatus,r.requestedBy,r.approvedBy,r.approvalDate,r.contractReq?"Yes":"No",r.contractSigned?"Yes":"No",r.grantable?"Yes":"No",r.grantPct,(r.grantable?(r.cost*r.grantPct/100):0).toFixed(2)].map(x=>`"${String(x??"").replace(/"/g,'""')}"`).join(",")));
    const blob=new Blob([lines.join("\n")],{type:"text/csv"}); const u=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=u; a.download="convention_expense_tracker.csv"; a.click(); URL.revokeObjectURL(u); };
  return <div className="space-y-5">
    <SectionTitle sub="Log every committed cost with full classification and QuickBooks account lines. Grant amount auto-calculates from grantable % × cost.">Expense Tracker</SectionTitle>
    <div className="grid gap-3" style={{gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))"}}>
      <Kpi label="Logged spend" value={usd(spent)} tone={C.deep}/><Kpi label="Entries" value={filtered.length} tone={C.nile}/><Kpi label="Grant-eligible $" value={usd(grantAmt)} tone={C.gold}/><Kpi label="Awaiting approval" value={tracker.filter(r=>r.pstatus==="Requested").length} tone={C.muted}/>
    </div>
    <div className="flex flex-wrap items-center gap-3">
    ...
    <Card pad={false} style={{overflow:"hidden"}}>
      <div className="overflow-x-auto"><table className="text-sm" style={{borderCollapse:"collapse",minWidth:1900}}>
        <thead><tr style={{background:C.deep,color:"#cfe0da"}} className="text-[11px] uppercase tracking-wide">{TRK_COLS.map(c=><th key={c[0]} className="text-left font-semibold px-3 py-2 whitespace-nowrap">{c[1]}</th>)}<th className="px-3 py-2 text-left font-semibold whitespace-nowrap">Grant $</th><th></th></tr></thead>
        <tbody>
          {filtered.length===0 && <tr><td colSpan={TRK_COLS.length+2} className="px-4 py-10 text-center" style={{color:C.muted}}>No expenses logged yet. Add your first committed cost to start tracking.</td></tr>}
...
function Foundation({expenses,income,settings}){
  const fLines=expenses.filter(r=>r.entity==="Foundation");
  const obligation=fLines.reduce((a,r)=>a+effEstimated(r,settings),0); const paid=fLines.reduce((a,r)=>a+(r.actual||0),0);
  ...
  return <div className="space-y-5">
    <SectionTitle sub="Everything tagged “Foundation” as the paying entity, netted against the Foundation reimbursement budgeted in Income. This is what the Foundation owes versus what the Fraternity is fronting.">Foundation Reimbursement Reconciliation</SectionTitle>
    ...
    <Card pad={false}>
      ...
      <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr style={{color:C.muted}} className="text-[11px] uppercase">{["Category","Line item","Estimate","Actual","Status"].map((h,i)=><th key={i} className="text-left font-semibold px-3 py-2" style={{borderBottom:`1px solid ${C.line}`}}>{h}</th>)}</tr></thead>
        <tbody>{fLines.map(r=>{ const eff=effEstimated(r,settings); const st=(r.actual||0)>=eff&&eff>0?"Settled":((r.actual||0)>0?"Partial":"Outstanding");
          return <tr key={r.id} style={{borderBottom:`1px solid ${C.line}`}}><td className="px-3 py-2" style={{color:C.muted}}>{r.category.slice(0,28)}</td><td className="px-3 py-2 font-medium">{r.item}</td><td className="px-3 py-2">{usd(eff)}</td><td className="px-3 py-2">{usd(r.actual)}</td><td className="px-3 py-2"><Pill text={st} color={st==="Settled"?C.pos:(st==="Partial"?C.warn:C.muted)}/></td></tr>; })}
